...
03_Series.reset_index(name="count") 支持这个参数：

它的作用是：

给这个 Series 里的数值起一个列名。
...
但普通 DataFrame 的 reset_index() 一般不能这样用：

df.reset_index(name="count")  # 通常会报错

因为 DataFrame 本身已经有列名，不需要用 name 给数值列命名。
